...
The Pokémon mini is a hand held created by Nintendo R&D3 around 1999 (released in 2001). It is based around the Epson S1C88 Core (then Timex/Seiko), and it derives most peripherals from this system.
...
- [CPU: S1C88V20](cpu/README.md)
  ...
  - [The Memory Map](cpu/memory.md)
    - [The Hardware Registers](cpu/registers)
...
- [Pokémon mini Circuit Board](board.md)
  - [Piezo Speaker](board.md#speaker)
  - [Rumble Motor](board.md#motor)
...
External bus with 21 bits for address (11+2 physically) & 8 bits for data.
...
- [*Internal EEPROM*](eeprom.md): 8192 bytes, read/write access via directly driving a two-wire I2C interface, used to store game saves.
- [*Cartridge ROM*](cpu/memory.md#cartridge-memory): Up to 2 MiB addressable (16 megabits). Official carts have a 512 KiB mask ROM.
...
- Tiled backgrounds: 8x8 pixel tiles, 4 different map sizes which expand beyond the screen boundaries and a scrollable display area (which does not loop automatically). The entire tilemap can be color inverted.
...
Can read back timer values (since sound is assigned to Timer C).

For commercial games, hold the C button while booting up to start muted. <!-- software comment but eh? -->

## [Timers](cpu/timers.md)

There are two oscillators and several timers in the system.

OSC1 is 32.768 kHz and is a low-power oscillator which cannot be disabled.
...
1.5V (converted to 3V internally) with 1 AAA Battery
