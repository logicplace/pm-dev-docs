...
| [4Ch][]    | $07E9 | Set startup LCD contrast                      |
| [4Eh][]    | $0802 | Step & apply startup contrast                 |
| [50h][]    | $081B | Apply startup LCD contrast                    |
| [52h][]    | $0821 | Get startup contrast                          |
...
[48h]: #shutdown-system
[4Ah]: #sleep
[4Ch]: #set-startup-lcd-contrast
[4Eh]: #step--apply-startup-contrast
[50h]: #apply-startup-lcd-contrast
[52h]: #get-startup-contrast
...
[$2000]: registers/lcd.md#startup-contrast
[$2001]: registers/multicart.md
...
### Set startup LCD contrast

Called via:

```s1c88
LD A, #(new startup contrast)
INT [4Ch]
; contents of B lost
```

This sets the [startup contrast][$2000] then applies it. `A` can be 0~63 (inclusive) where 0 is fully white and 63 is fully black. The startup contrast set by the bios is 31 (50%). The LCD panel itself normally defaults to 32.
...
This decrements or increments the startup contrast by 1 then applies it.

When accessing LCD commands like this, it's a good idea to disable interrupts (other than ones leading to a system shutdown).

While official software (such as [Shock Tetris][]) do step the contrast by 1, no software does so via this software interrupt.

### Apply startup LCD contrast

Called via `INT [50h]`

This restores startup contrast from [$2000][]. Contents of `B` are lost.

When accessing LCD commands like this, it's a good idea to disable interrupts (other than ones leading to a system shutdown).

No official software uses this.

### Get startup contrast

Called via:

```
INT [52h]
; startup contrast is now in A
```

No official software uses this. Instead, [MINLIB][] provides a method for this (TODO: link?) which respects the different location of the startup contrast on a development rig.
...
We call this "temporary" because [50h][] is able to restore the startup, which this doesn't edit. `A` can be 0~63 (inclusive) where 0 is fully white and 63 is fully black. The startup contrast set by the bios is 31 (50%). The LCD panel itself normally defaults to 32.
...
If bit 7 of [$2001](registers/01-7.md) is unset, sets bits 7~6 and resets bit 5, which sets the console as "asleep", which will cause IRQs to follow the [wake up](./interrupts.md#waking-up) pathway.
...
If bit 6 of [$2001](registers/01-6.md) is unset, prepares [cart eject][], resets bits 7~6, and sets bit 5, which will cause IRQs to stop following [wake up](./interrupts.md#waking-up) pathway
